...
ROCMLIR_RELEASE_COMMIT=41251784f1780270545974c832c1093fb4a8d1c1
...
ROCMLIR_LICENSE="${ROOT}/tools/sam3_fc1sink/rocmlir-LICENSE"
PATCH_SHA256=2739601912d21c8970244eacc25db8e897bc67043497be53c3725a413f57ad5a
ARCHIVE_NAME="migraphx-2.17.0-dev-${BASE_SHORT}-sam3-fc1sink-rocm${ROCM_VERSION}-\
${GPU_ARCH}-${PYTHON_ABI}.tar.gz"
PACKAGE_ONLY=""
RESUME=0
CHECK_ONLY=0
...
PYTHON_ABI, BUILD_ROOT, DEPS_DIR, OUTPUT_DIR, and ROCMLIR_SOURCE. Point
ROCMLIR_SOURCE at a local rocMLIR checkout for an offline source check.
EOF
}
...
for command in git gzip nproc python3 readelf sha256sum tar; do
...
[[ -f "${ROCMLIR_LICENSE}" ]] || die "missing rocMLIR license: ${ROCMLIR_LICENSE}"
...
mkdir -p "${doc_root}/amdrocm-migraphx" "${doc_root}/rocmlir" \
         "${doc_root}/sam3-rocmlir"
cp "${ROOT}/LICENSE" "${doc_root}/amdrocm-migraphx/LICENSE"
cp "${ROCMLIR_LICENSE}" "${doc_root}/rocmlir/LICENSE"
...
tar --sort=name \
    ...
    -cf - -C "${staging}" migraphx | gzip -n -9 >"${archive}"
